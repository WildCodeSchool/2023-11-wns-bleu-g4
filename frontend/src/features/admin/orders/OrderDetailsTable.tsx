...
export default function OrderDetailsTable({ bookingItems }: { bookingItems: BookingItem[] | undefined }) {
  ...
  return (
    <td colSpan={7} className="p-4 align-top border-y border-gray-400">
      ...
      <table className="w-full min-w-full">
        <thead>
          <tr>
            {orderDetailsHeaders.map(menu => (
              <th
                className="h-14 p-3 first:pl-0 last:pr-8 text-left uppercase text-sm font-bold 
                whitespace-nowrap border-b border-gray-400"
                key={menu.id}
              >
        ...
        <tbody className="text-sm">
          {bookingItems?.map((bookingItem: BookingItem) => (
            <tr key={bookingItem.id} className="border-y border-t-gray-400 border-b-0">
