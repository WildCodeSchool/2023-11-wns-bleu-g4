...
export default function ImageCarousel() {
    const renderArrowPrev = (onClickHandler: () => void, hasPrev: boolean, label: string) =>
        hasPrev && (
            <button type="button" onClick={onClickHandler} title={label} className="absolute z-10 top-1/2 left-4 transform -translate-y-1/2 w-12 h-12 bg-zinc-50 rounded-3xl cursor-pointer flex items-center justify-center">
    ...
    const renderArrowNext = (onClickHandler: () => void, hasNext: boolean, label: string) =>
        hasNext && (
            <button type="button" onClick={onClickHandler} title={label} className="absolute z-10 top-1/2 right-4 transform -translate-y-1/2 w-12 h-12 bg-zinc-50 rounded-3xl cursor-pointer flex items-center justify-center">
