...
export default function CheckPasswordAvailability({ password, secondPassword }: PasswordCriteriaType) {
    ...
    const checkList = [
        ...
        { text: t("At least 12 characters" + (password.length > 0 ? " (" + password.length + ")" : "")), isAvailable: twelveChars },
                ...
                checkList.map((el, i) => {
                    return (
                        <ListItem key={i} className={`flex items-center ${el.isAvailable ? "text-green-600  " : "text-red-500"}`} >
                            <ListIcon as={el.isAvailable ? CheckIcon : XMarkIcon} color={el.isAvailable ? 'text-green-600' : "text-red-500"} />
                            {el.text}
                        </ListItem>)
                })
            }
