...
export default function UserInfos({ user }: { user?: User }) {
  ...
  let bgTableHeadColor = useColorModeValue(
    transparentBackGround ? "transparent" : "#d0d2d6",
    transparentBackGround ? "transparent" : "cactus.900",
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/*************************** BUTTONS ***************************/}
          <Flex
            className="w-full py-3 px-5 gap-4 justify-between items-start h-fit 
          ...
          >
            {/*************************** UPDATE ***************************/}
            <Button className="w-1/2" size="xs" padding="4" onClick={toggleUpdateUserModal} variant={"accentButton"}>
              Update
            </Button>
            <UserInfoModal isOpen={isUpdateModalOpen} onClose={toggleUpdateUserModal} user={user} />

            {/*************************** DELETE ***************************/}
            <Button className="w-1/2" size="xs" padding="4" variant={"warningButton"} onClick={toggleDeleteUserModal}>
              Delete Account
            </Button>
