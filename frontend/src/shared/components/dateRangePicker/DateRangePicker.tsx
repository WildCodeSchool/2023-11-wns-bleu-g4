...
const DateRangePicker = ({
  ...
  const handleSelectRange: SelectRangeEventHandler = useCallback(
    selectedRange => {
      const isRangeBooked =
        selectedRange?.from &&
        selectedRange.to &&
        bookedDays.some(bookedRange => {
          return isBefore(selectedRange.from!, bookedRange.to) && isAfter(selectedRange.to!, bookedRange.from);
        });
  ...
  const positionStyle = useMemo(
    () => (position === "down" ? { top: "calc(100% + 10px)" } : { bottom: "calc(100% + 10px)" }),
    [position],
  );
